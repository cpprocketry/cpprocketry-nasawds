...
gulp.task(task, [ 'copy-vendor-sass' ], function () {
  ...
  var stream = gulp.src('src/stylesheets/nasawds.scss')
  ...
  stream
    ...
    // 3. rename to nasawds.min.css
    .pipe(rename({
      suffix: '.min',
    }))
    // 4. write dist/css/nasawds.min.css.map
    .pipe(sourcemaps.write('.'))
    // 5. write dist/css/nasawds.min.css
    .pipe(gulp.dest('dist/css'));
